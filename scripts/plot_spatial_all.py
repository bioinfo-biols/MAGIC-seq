...
def plot_spatial_general(
    ...
    if value_df.shape[1] > 64:
        raise ValueError("Maximum of 16 cell types / factors can be plotted at the moment")
    ...
    cmaps = [create_colormap(hex2rgb_3(i)[0],hex2rgb_3(i)[1],hex2rgb_3(i)[2]) for i in cmaps]
    ...
    with mpl.style.context(style):
        ...
        if colorbar_position == "right":
            ...
            shape = {"vertical_gaps": 2.5, "horizontal_gaps": 0, "width": 0.15, "height": 0.2}
        ...
        for c in c_ord:
            ...
            min_color_intensity = np.max([np.quantile(counts[:, c], min_color_quantile), 0.1])
...
def plot_spatial_all(adata, color, img_key="hires", show_img=True, **kwargs):
...
def plot_Multiple(
    ...
    value_df=None,
    coords = None,
    cmaps=['#10976f','#cac43d','#ec7c7f','#865123',
        '#ffbe2c','#67c1dd','#f0e442','#b71e75',
        '#1681ec','#009e73','#7cc07c','#a81e2b',
        '#56b4e9','#57139f','#d7d9a5','#7f9745',
        '#3e7da5','#8358f3','#f29f87','#e9f0c5',
        '#f7d0e1','#c0e2da','#f29f87','#4f80ff',
        '#56bb7f','#b9b9d7','#30817a','#d55e00',
        '#80b1d3','#fb8072','#ccebc5','#d6d3e7'],
    circle_diameter=4.0,
    alpha_scaling=1,
    ...
    colorbar_label_kw={"size": 15, "alpha": 1},
    colorbar_shape={"vertical_gaps": 3.5, "horizontal_gaps": 0, "width": 0.15, "height": 0.2},
    colorbar_tick_size=12,
    colorbar_grid=None,
    img=None,
    img_alpha=None,
    ...
    cmaps = [create_colormap(hex2rgb_3(i)[0],hex2rgb_3(i)[1],hex2rgb_3(i)[2]) for i in cmaps]
    cmaps = [cmaps[i] for i in range(len(cmaps))]
    max_col=[np.inf]*len(cmaps)
    ...
    with mpl.style.context(style):
        ...
        if colorbar_position == "right":
            ...
            gs = GridSpec(
                nrows=colorbar_grid[0] + 2,
                ncols=colorbar_grid[1] + 1,
                width_ratios=[1, *[colorbar_shape["width"]] * colorbar_grid[1]],
                height_ratios=[1, *[colorbar_shape["height"]] * colorbar_grid[0], 1],
                hspace=colorbar_shape["vertical_gaps"],
                wspace=colorbar_shape["horizontal_gaps"],
            )
            ax = fig.add_subplot(gs[:, 0], aspect="equal", rasterized=True)
        ...
        for c in c_ord:
            ...
            if min_color_value is not None:
                min_color_intensity = min_color_value[c] #min_color_intensity = np.max([np.quantile(counts[:, c], min_color_quantile), 0.1])
            if max_color_value is not None:
                max_color_intensity = max_color_value[c]
            print(labels[c],'min_max_color:',round(min_color_intensity,2),'-',round(max_color_intensity,2))
            value_df[labels[c]][value_df[labels[c]]<min_color_intensity]=min_color_intensity
            value_df[labels[c]][value_df[labels[c]]>max_color_intensity]=max_color_intensity
            ...
            if colorbar_position is not None:
                ...
                cbar.ax.tick_params(labelsize=colorbar_tick_size)
                max_color = rgb_function(max_color_intensity / 1.5)
                cbar.ax.set_title(labels[c], **{**{"color": max_color}, **colorbar_label_kw})
        ...
        if coords is not None:
            ax.scatter(x=coords[:, 0], y=coords[:, 1], c=weighted_colors, s=circle_diameter**2)
    return fig,weighted_colors,value_df
